...
$querySechoire = "SELECT 
    CONCAT(DATE_SUB(DATE(s.date_fin_sechoire), INTERVAL 1 DAY), ' 22:00:00') as debut_poste1,
    CONCAT(DATE(s.date_fin_sechoire), ' 06:00:00') as fin_poste1,
    CONCAT(DATE(s.date_fin_sechoire), ' 14:00:00') as fin_poste2,
    CONCAT(DATE(s.date_fin_sechoire), ' 22:00:00') as fin_poste3,
    SUM(
        CASE 
            WHEN s.date_fin_sechoire BETWEEN CONCAT(DATE_SUB(DATE(s.date_fin_sechoire), INTERVAL 1 DAY), ' 22:00:00') AND CONCAT(DATE(s.date_fin_sechoire), ' 06:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 22h à 6h du matin
            ELSE 0
        END
    ) as poste_1,
    SUM(
        CASE 
            WHEN s.date_fin_sechoire BETWEEN CONCAT(DATE(s.date_fin_sechoire), ' 06:00:00') AND CONCAT(DATE(s.date_fin_sechoire), ' 14:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 6h à 14h
            ELSE 0
        END
    ) as poste_2,
    SUM(
        CASE 
            WHEN s.date_fin_sechoire BETWEEN CONCAT(DATE(s.date_fin_sechoire), ' 14:00:00') AND CONCAT(DATE(s.date_fin_sechoire), ' 22:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 14h à 22h
            ELSE 0
        END
    ) as poste_3,
    DATE(s.date_fin_sechoire) as jour,
    SUM(p.metrage_dispo) as total_metrage
FROM 
    sechoire s
LEFT JOIN 
    preparation p ON s.dispo = p.dispo AND s.sub =p.sub
WHERE 
    s.etat = 'termine'  AND s.date_fin_sechoire != '0000-00-00 00:00:00'
GROUP BY 
    DATE(s.date_fin_sechoire)";
...
$queryRameuse = "SELECT 
    CONCAT(DATE_SUB(DATE(r.date_fin_rameuse), INTERVAL 1 DAY), ' 22:00:00') as debut_poste1,
    CONCAT(DATE(r.date_fin_rameuse), ' 06:00:00') as fin_poste1,
    CONCAT(DATE(r.date_fin_rameuse), ' 14:00:00') as fin_poste2,
    CONCAT(DATE(r.date_fin_rameuse), ' 22:00:00') as fin_poste3,
    SUM(
        CASE 
            WHEN r.date_fin_rameuse BETWEEN CONCAT(DATE_SUB(DATE(r.date_fin_rameuse), INTERVAL 1 DAY), ' 22:00:00') AND CONCAT(DATE(r.date_fin_rameuse), ' 06:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 22h à 6h du matin
            ELSE 0
        END
    ) as poste_1,
    SUM(
        CASE 
            WHEN r.date_fin_rameuse BETWEEN CONCAT(DATE(r.date_fin_rameuse), ' 06:00:00') AND CONCAT(DATE(r.date_fin_rameuse), ' 14:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 6h à 14h
            ELSE 0
        END
    ) as poste_2,
    SUM(
        CASE 
            WHEN r.date_fin_rameuse BETWEEN CONCAT(DATE(r.date_fin_rameuse), ' 14:00:00') AND CONCAT(DATE(r.date_fin_rameuse), ' 22:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 14h à 22h
            ELSE 0
        END
    ) as poste_3,
    DATE(r.date_fin_rameuse) as jour,
    SUM(p.metrage_dispo) as total_metrage
FROM 
    rameuse r
LEFT JOIN 
...
WHERE 
    r.etat =  'termine' AND r.date_fin_rameuse != '0000-00-00 00:00:00'
GROUP BY 
    DATE(r.date_fin_rameuse)";


$resultRameuse = mysqli_query($conn, $queryRameuse);

$queryTeinture = "SELECT 
    CONCAT(DATE_SUB(DATE(t.date_fin_teinture), INTERVAL 1 DAY), ' 22:00:00') as debut_poste1,
    CONCAT(DATE(t.date_fin_teinture), ' 06:00:00') as fin_poste1,
    CONCAT(DATE(t.date_fin_teinture), ' 14:00:00') as fin_poste2,
    CONCAT(DATE(t.date_fin_teinture), ' 22:00:00') as fin_poste3,
    SUM(
        CASE 
            WHEN t.date_fin_teinture BETWEEN CONCAT(DATE_SUB(DATE(t.date_fin_teinture), INTERVAL 1 DAY), ' 22:00:00') AND CONCAT(DATE(t.date_fin_teinture), ' 06:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 22h à 6h du matin
            ELSE 0
        END
    ) as poste_1,
    SUM(
        CASE 
            WHEN t.date_fin_teinture BETWEEN CONCAT(DATE(t.date_fin_teinture), ' 06:00:00') AND CONCAT(DATE(t.date_fin_teinture), ' 14:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 6h à 14h
            ELSE 0
        END
    ) as poste_2,
    SUM(
        CASE 
            WHEN t.date_fin_teinture BETWEEN CONCAT(DATE(t.date_fin_teinture), ' 14:00:00') AND CONCAT(DATE(t.date_fin_teinture), ' 22:00:00') THEN 
                p.metrage_dispo  -- Poste de travail de 14h à 22h
            ELSE 0
        END
    ) as poste_3,
    DATE(t.date_fin_teinture) as jour,
    SUM(p.metrage_dispo) as total_metrage
FROM 
    teinture t
LEFT JOIN 
...
WHERE 
    t.etat = 'termine' AND t.date_fin_teinture != '0000-00-00 00:00:00'
GROUP BY 
    DATE(t.date_fin_teinture)";
...
function getTotalMetrageByDate($conn) {
    ...
    $etapes = array("sfaldina", "teinture", "essorage", "sechoire", "rameuse", "spazzolato");
