...
<body>
  ...
  <option ></option>
	...
	<option >en cours reparation</option>
	<option >fin reparation</option>
  </select>
</div>
                    <div class="form-group">
                  ...
                  <table id="myTable" class="table table-hover">
                                            <thead class="text-dark fs-4">
                                            	...
                                            	<th class="border-bottom-0">
													...
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Debut Teinture</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Fin teinture</h6>
                                                    </th>
													...
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">DeltaE 2</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Heure Campionatura 2</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Debut R&eacute;paration 2</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Fin R&eacute;paration 2</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">DeltaE 3</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Heure Campionatura 3</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Debut R&eacute;paration 3</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Fin R&eacute;paration 3</h6>
                                                    </th>
													
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">DeltaE Finale</h6>
                                                    </th>
...
$query = "SELECT DISTINCT
 ...
 s.deltaE 
          ...
          LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub ORDER BY date_debut_teinture DESC " ;
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $duree_teinture = strtotime($date_fin_teinture) - strtotime($date_debut_teinture);
	$duree_teinture_heur =  round($duree_teinture /3600);
	$duree_teinture_vide = strtotime($date_actuelle) - strtotime($date_debut_teinture);
	$duree_teinture_vide_heur =  round($duree_teinture_vide /3600);
	
	
    $temps_attente = strtotime($date_debut_teinture) - strtotime($date_heur_pesage);
    ...
    echo "<tr>";
		...
		if ($date_fin_teinture != "0000-00-00 00:00:00"){
		if ($duree_teinture_heur > "8") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_teinture_heur Heures</span></td>";
        } elseif ($duree_teinture_heur === "8") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_teinture_heur Heures</span></td>";
        }elseif ($duree_teinture_heur == "0") {
            echo "<td class='border-bottom-0'><span class='badge bg-warning rounded-3 fw-semibold'>0 Heures</span></td>";
        }elseif ($duree_teinture_heur > "0") {
            echo "<td class='border-bottom-0'><span class='badge bg-warning rounded-3 fw-semibold'>$duree_teinture_heur Heures</span></td>";
        }
		} else{
			echo "<td class='border-bottom-0'><span class='badge bg-warning rounded-3 fw-semibold'>$duree_teinture_vide_heur Heures</span></td>";
		}

		echo "<td class='border-bottom-0'>" . $row['deltaE2'] . "</td>";
		echo "<td class='border-bottom-0'>" . $row['heure_campionatura2'] . "</td>";
        echo "<td class='border-bottom-0'>" . $row['date_debut_reparation1'] . "</td>";
        echo "<td class='border-bottom-0'>" . $row['date_fin_reparation1'] . "</td>";
		echo "<td class='border-bottom-0'>" . $row['deltaE3'] . "</td>";
		echo "<td class='border-bottom-0'>" . $row['heure_campionatura3'] . "</td>";
        echo "<td class='border-bottom-0'>" . $row['date_debut_reparation2'] . "</td>";
        echo "<td class='border-bottom-0'>" . $row['date_fin_reparation2'] . "</td>";
		
		
        if ($deltaE <= "1.2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$deltaE</span></td>";
        } else {
            echo "<td class='border-bottom-0'><span  class='badge bg-danger rounded-3 fw-semibold'>$deltaE</span></td>";
        } 
...
document.getElementById("exportButton").addEventListener("click", function() {
    ...
    var url = `extraction-teinture.php?filterInput=${filterInput}&filterColor=${filterColor}&filterArticle=${filterArticle}&filterEtat=${filterEtat}&Date=${Date}&startDate=${startDate}&endDate=${endDate}`;
...
function filterTableDispo() {
    ...
    for (i = 0; i < tr.length; i++) {
        ...
        var dateCell = tr[i].getElementsByTagName("td")[10]; // Index 12 correspond à la colonne "Date Pesage"
