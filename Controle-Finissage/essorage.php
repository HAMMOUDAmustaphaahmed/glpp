  ...
  <div class="card-body">
                        <div class="col-xl-12  d-flex ">
                                ...
                                <div class="card-body p-4">
                                    <h5 class="card-title fw-semibold mb-4">Pilotage Essorage</h5>
                  ...
                  <table id="myTable" class="table table-hover">
                                            ...
                                            <thead class="text-dark fs-4">
												...
												<th class="border-bottom-0">
													...
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Debut Essorage</h6>
                                                    </th>
													
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Fin Essorage</h6>
                                                    </th>
                                                    <th class="border-bottom-0">
...
$query = "SELECT  p.dispo,
 ...
 p.poids_reel,
 p.metrage_dispo,
 s.date_debut_essorage,
 s.date_fin_essorage,
 s.machine,
 s.operateur,
 s.etat,
 sf.date_fin_teinture
 FROM `essorage` s
 LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub
 LEFT JOIN `teinture` sf ON s.dispo = sf.dispo AND s.sub = sf.sub
 WHERE s.date_debut_essorage !='0000-00-00 00:00:00' 
 ORDER BY s.date_debut_essorage DESC";
...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  $date_actuelle = date('Y-m-d H:i:s', strtotime('+1 hour'));
    ...
    $duree_essorage = strtotime($date_fin_essorage) - strtotime($date_debut_essorage);
	$duree_essorage_heur =  round($duree_essorage /3600);
	
	$temps_attente = strtotime($date_debut_essorage) - strtotime($date_fin_teinture);
	$temps_attente_heur =round($temps_attente/3600);
	$temps_attente_empty = strtotime($date_actuelle) - strtotime($date_fin_teinture);
...
if ($date_fin_essorage != "0000-00-00 00:00:00"){
...
document.getElementById("exportButton").addEventListener("click", function() {
    ...
    var url = `extraction-essorage.php?filterInput=${filterInput}&filterColor=${filterColor}&filterArticle=${filterArticle}&filterEtat=${filterEtat}&Date=${Date}&startDate=${startDate}&endDate=${endDate}`;
...
function filterTableDispo() {
    ...
    for (i = 0; i < tr.length; i++) {
        ...
        var dateCell = tr[i].getElementsByTagName("td")[12]; // Index 12 correspond à la colonne "Date Pesage"
