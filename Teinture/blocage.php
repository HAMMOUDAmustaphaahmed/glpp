...
if (isset($_POST['Submit'])) {
    ...
    // Mettre à jour les données dans la table `blocage`
        ...
        if ($dispo != '' && $sub != '') {
            ...
            $checkQuery1 = "SELECT * FROM teinture1 WHERE dispo = '$dispo' AND sub = '$sub' ";
            $result1 = mysqli_query($conn, $checkQuery1);

            if (mysqli_num_rows($result1) > 0) {
                // Les conditions sont remplies dans teinture1, mettez à jour teinture1
				$etape = "teinture1";
                $updateQuery = "UPDATE `teinture1` SET etat = '$etat', date_fin_teinture1 = '$date_blocage' WHERE dispo = '$dispo' AND sub = '$sub'";
                ...
                // Si les conditions ne sont pas remplies dans teinture1, vérifiez teinture2
				
                $checkQuery2 = "SELECT * FROM teinture2 WHERE dispo = '$dispo' AND sub = '$sub' ";
                $result2 = mysqli_query($conn, $checkQuery2);
                 
                if (mysqli_num_rows($result2) > 0) {
                    // Les conditions sont remplies dans teinture2, mettez à jour teinture2
					$etape = "teinture2";
                    $updateQuery = "UPDATE `teinture2` SET etat = '$etat', date_fin_teinture2 = '$date_blocage' WHERE dispo = '$dispo' AND sub = '$sub'";
					$insertQuery = "INSERT INTO `blocage` (dispo, sub, etape,etat, date_blocage, details_blocage, operateur) VALUES ('$dispo', '$sub', '$etape','$etat', '$date_blocage', '$details_blocage','$operateur')";
                }
            }
        }
...
<?php
        ...
        $sql1 = "SELECT DISTINCT dispo FROM teinture1 WHERE etat = 'en cours'";
        ...
        $sql2 = "SELECT  distinct dispo FROM teinture2 WHERE etat = 'en cours'";
        ...
        ?>
          ...
          <select  class="form-select" name ="sub"  required>
            ...
            include('db.php');
        // Construct and execute a SQL query to fetch "dispos" from sechoire1
        $sql1 = "SELECT distinct sub FROM teinture1 WHERE etat = 'en cours' and dispo ='$dispo' ";
        $result1 = $conn->query($sql1);
        ...
        // Construct and execute a SQL query to fetch "dispos" from sechoire2
        $sql2 = "SELECT distinct sub FROM teinture2 WHERE etat = 'en cours' and dispo ='$dispo'" ;
        $result2 = $conn->query($sql2);
