...
FROM
    `rameuse` s 
LEFT JOIN
    `preparation` p ON s.dispo = p.dispo and s.sub = p.sub AND s.article = p.article and s.couleur = p.couleur
LEFT JOIN
    `sfaldina` k ON s.dispo = k.dispo and s.sub = k.sub
LEFT JOIN
    `teinture` t ON s.dispo = t.dispo and s.sub = t.sub
LEFT JOIN
    `essorage` e ON s.dispo = e.dispo and s.sub = e.sub
LEFT JOIN
    `spazzolato` r ON s.dispo = r.dispo and s.sub = r.sub
 
LEFT JOIN
    `sechoire` a ON s.dispo = a.dispo and s.sub = a.sub
LEFT JOIN 
    `ref` m ON s.article = m.article AND s.couleur = m.couleur AND m.article !='' AND m.couleur !=''
WHERE
    m.article = 'MT1'
