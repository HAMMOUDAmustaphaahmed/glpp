...
(UNIX_TIMESTAMP(NOW() ) - UNIX_TIMESTAMP(s.date_debut_teinture)) / 3600), 0) AS duree_heures,
s.etat,
s.deltaE2,
...
if (!empty($Date)) {
    $query .= " AND s.date_debut_teinture LIKE '%$Date%'";
}
if (!empty($startDate) && !empty($endDate)) {
    $query .= " AND s.date_debut_teinture >= '$startDate' AND s.date_debut_teinture <= '$endDate'";
}
