...
// Définissez le nom du fichier avec "pilotage_sechoire" suivi de la date actuelle
$filename = "Pilotage_sechoire_" . $currentDate . ".xls";
...
p.poids_reel,
p.metrage_dispo,
ROUND(IF(s.date_fin_sechoire != '0000-00-00 00:00:00',
(UNIX_TIMESTAMP(s.date_fin_sechoire) - UNIX_TIMESTAMP(s.date_debut_sechoire)) / 3600,
(UNIX_TIMESTAMP(NOW() + INTERVAL 1 HOUR) - UNIX_TIMESTAMP(s.date_debut_sechoire)) / 3600), 0) AS duree_heures,
...
if (!empty($Date)) {
    $query .= " AND s.date_fin_sechoire LIKE '%$Date%'";
}
if (!empty($startDate) && !empty($endDate)) {
    $query .= " AND s.date_fin_sechoire BETWEEN '$startDate' AND '$endDate'";
}
$query .= " ORDER BY s.date_debut_sechoire DESC";
...
$headers = array("Operateur", "Machine","Dispo", "Sub", "Article", "Couleur", "etat","Poids Dispo", "Poids Reel", "Metrage Dispo","Durée", "Date Debut sechoire",  "Date Fin sechoire","deltaE" );
