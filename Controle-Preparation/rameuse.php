...
<div class="container ">
                    ...
                    <div class="card-body">
                        <div class="col-xl-10 col-lg-6 d-flex col-md-8 mx-auto ">
                            
                                <div class="card-body p-4">
                                    <h5 class="card-title fw-semibold mb-4">Pilotage Rameuse</h5>
                  ...
                  <table id="myTable" class="table table-hover">
                                            ...
                                            <thead class="text-dark fs-4">
                                            	...
                                            	<th class="border-bottom-0">
													...
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Grammage</h6>
                                                    </th>
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Vitesse</h6>
                                                    </th>
													
													
                                                    
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Debut Rameuse</h6>
                                                    </th>
													
													<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Date Fin Rameuse</h6>
                                                    </th>
                                                   	<th class="border-bottom-0">
                                                        <h6 class="fw-semibold mb-0">Dur&eacute;e</h6>
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody>
...
$query1 = "SELECT 
 ...
 s.vitesse
          FROM `rameuse` s 
          LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub ORDER BY s.date_debut_rameuse DESC " ;
...
while ($row = mysqli_fetch_assoc($result1)) {
	...
	$date_fin_rameuse = $row['date_fin_rameuse'];
	$date_heur_pesage = $row['date_heur_pesage'];
    $duree_rameuse = strtotime($date_fin_rameuse) - strtotime($date_debut_rameuse);
	$duree_rameuse_heur =  round($duree_rameuse /3600);
        ...
        echo "<td class='border-bottom-0'>" . $row['grammage'] . "</td>";
		echo "<td class='border-bottom-0'>" . $row['vitesse'] . "</td>";
	
		echo "<td class='border-bottom-0'>" . $row['date_debut_rameuse'] . "</td>";
        echo "<td class='border-bottom-0'>" . $row['date_fin_rameuse'] . "</td>";
        if ($date_fin_rameuse != "0000-00-00 00:00:00"){
...
document.getElementById("exportButton").addEventListener("click", function() {
    ...
    var url = `extraction-rameuse.php?filterInput=${filterInput}&filterColor=${filterColor}&filterArticle=${filterArticle}&filterEtat=${filterEtat}&Date=${Date}&startDate=${startDate}&endDate=${endDate}`;
