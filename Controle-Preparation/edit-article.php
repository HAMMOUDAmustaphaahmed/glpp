...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT `article`, `groupe_article` FROM `ref` WHERE `id` = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "i", $editId);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $article, $groupe_article);
...
if (isset($_POST['Submit'])) {
    ...
    $groupe_article = $_POST['groupe_article'];

    // Update query
    $updateQuery = "UPDATE `ref` SET `article` = ?, `groupe_article` = ? WHERE `id` = ?";
    ...
    if (mysqli_stmt_execute($stmt)) {
        $alertClass = 'alert-success';
            $message = "article modifi&eacute; avec succ&eagrave;s";
    } else {
       $alertClass = 'alert-danger';
            $message = "Erreur lors de modification d'article : " . mysqli_error($conn);
    }
  ...
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
	...
	<label class="form-label" style="color: #4682B4;">Ajouter Article</label>
        ...
        <div class="col">
          ...
          <input type="text" class="form-control"   name = "article" value="<?php echo $article ?>">
          </div>
		  <div class="col">
          <label  class="form-label">Groupe Article</label>
          <input type="text" class="form-control"   name = "groupe_article" value="<?php echo $groupe_article ?>">
          </div>
