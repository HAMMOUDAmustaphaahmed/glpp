...
if (isset($_POST['Submit'])) {
    ...
    $date_debut_spazzolato = date('Y-m-d H:i:s', strtotime('+1 hour'));
    $etat = "en cours";
                $updateSpazzolatoQuery = "UPDATE `spazzolato` SET  operateur = '$operateur', etat='$etat', date_debut_spazzolato='$date_debut_spazzolato' WHERE dispo='$dispo' AND sub='$sub'";

                  if (mysqli_query($conn, $updateSpazzolatoQuery)) {
...
if (isset($_POST['Su'])) {
    ...
    $date_fin_spazzolato = date('Y-m-d H:i:s', strtotime('+1 hour'));
    $etat = "termine";


                // Mettre \u00e0 jour les donn\u00e9es dans la table `spazzolato`
                $updateSpazzolatoQuery = "UPDATE `spazzolato` SET etat='$etat', date_fin_spazzolato='$date_fin_spazzolato' WHERE dispo='$dispo' AND sub='$sub'";
