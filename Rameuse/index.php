...
<body>
  ...
  <label for="filterInput3">Filtrer par Operateur :</label>
...
// Première requête
$query1 = "SELECT DISTINCT p.id AS preparation_id, s.dispo, s.sub,s.operateur, s.article, s.couleur, s.etat, sf.date_fin_sechoire,s.date_debut_rameuse, s.etape
          FROM `rameuse` s 
          LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub  
          LEFT JOIN `sechoire` sf ON s.dispo = sf.dispo AND s.sub = sf.sub  
          WHERE sf.date_fin_sechoire != '0000-00-00 00:00:00' 
		   AND (s.article != 'ATN' AND s.article != 'AOU' AND s.article != 'VR5' AND s.article != 'YN4')";


// Deuxième requête
$query2 = "SELECT DISTINCT p.id AS preparation_id, s.dispo, s.sub,s.operateur, s.article, s.couleur, s.etat,s.date_debut_rameuse, sf.date_fin_spazzolato ,s.etape
          FROM `rameuse` s 
          LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub 
          LEFT JOIN `spazzolato` sf ON s.dispo = sf.dispo AND s.sub = sf.sub 
          LEFT JOIN `sechoire` d ON s.dispo = d.dispo AND s.sub = d.sub AND d.etat = 'sechoire1'
          WHERE sf.date_fin_spazzolato  != '0000-00-00 00:00:00'  
		  ";
// troisième requête
$query3 = "SELECT DISTINCT p.id AS preparation_id, s.dispo, s.sub,s.operateur, s.article, s.couleur, s.etat,s.date_debut_rameuse, sf.date_fin_sfaldina ,s.etape,p.type_traitement 
          FROM `rameuse` s 
          LEFT JOIN `preparation` p ON s.dispo = p.dispo AND s.sub = p.sub
          LEFT JOIN `sfaldina` sf ON s.dispo = sf.dispo AND s.sub = sf.sub
          WHERE sf.date_fin_sfaldina  != '0000-00-00 00:00:00'
		   AND p.type_traitement ='thermofixage'";
...
while ($row = mysqli_fetch_assoc($result1)) {
    ...
    $duree_sechoire_rameuse = strtotime($date_actuelle) - strtotime($date_fin_sechoire);
	$duree_sechoire_rameuse_heur =  round($duree_sechoire_rameuse /3600);
	 $duree_sechoire_rameuse_not = strtotime($date_debut_rameuse) - strtotime($date_fin_sechoire);
	$duree_attente =  round($duree_sechoire_rameuse_not /3600);
    echo "<tr>";
    ...
    echo "<td class='border-bottom-0'>" . $row['date_fin_sechoire'] . "</td>";
		...
		if ($etat == 'en attente') {
	if ($duree_sechoire_rameuse_heur >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_sechoire_rameuse_heur Heures</span></td>";
        } elseif ($duree_sechoire_rameuse_heur < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_sechoire_rameuse_heur Heures</span></td>";
        } 
	} else {
	    if ($duree_attente >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_attente Heures</span></td>";
        } elseif ($duree_attente < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_attente Heures</span></td>";
        }
...
while ($row = mysqli_fetch_assoc($result2)) {
	...
	$date_actuelle = date('Y-m-d H:i:s', strtotime('+1 hour'));
	$date_debut_rameuse = $row['date_debut_rameuse'];
    $duree_spazzolato_rameuse_empty = strtotime($date_actuelle) - strtotime($date_fin_spazzolato);
	$duree_spazzolato_rameuse_empty_heur =  round($duree_spazzolato_rameuse_empty /3600);
	
	$duree_spazzolato_rameuse = strtotime($date_debut_rameuse) - strtotime($date_fin_spazzolato);
	$duree_spazzolato_rameuse_heur =  round($duree_spazzolato_rameuse /3600);
    ...
    echo "<td class='border-bottom-0'>" . $row['date_fin_spazzolato'] . "</td>";
		if ($etat == 'en attente') {
	if ($duree_spazzolato_rameuse_empty_heur >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_spazzolato_rameuse_empty_heur Heures</span></td>";
        } elseif ($duree_spazzolato_rameuse_empty_heur < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_spazzolato_rameuse_empty_heur Heures</span></td>";
        } 
	} else {
	    if ($duree_spazzolato_rameuse_heur >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_spazzolato_rameuse_heur Heures</span></td>";
        } elseif ($duree_spazzolato_rameuse_heur < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_spazzolato_rameuse_heur Heures</span></td>";
        }
...
while ($row = mysqli_fetch_assoc($result3)) {
	...
	$date_actuelle = date('Y-m-d H:i:s', strtotime('+1 hour'));
	$date_debut_rameuse = $row['date_debut_rameuse'];
    $duree_sfaldina_rameuse_empty = strtotime($date_actuelle) - strtotime($date_fin_sfaldina);
	$duree_sfaldina_rameuse_empty_heur =  round($duree_sfaldina_rameuse_empty /3600);
	
	$duree_sfaldina_rameuse = strtotime($date_debut_rameuse) - strtotime($date_fin_sfaldina);
	$duree_sfaldina_rameuse_heur =  round($duree_sfaldina_rameuse /3600);
    ...
    echo "<td class='border-bottom-0'>" . $row['date_fin_sfaldina'] . "</td>";
		...
		if ($etat == 'en attente') {
	if ($duree_sfaldina_rameuse_empty_heur >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_sfaldina_rameuse_empty_heur Heures</span></td>";
        } elseif ($duree_sfaldina_rameuse_empty_heur < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_sfaldina_rameuse_empty_heur Heures</span></td>";
        } 
	} else {
	    if ($duree_sfaldina_rameuse_heur >= "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-danger rounded-3 fw-semibold'>$duree_sfaldina_rameuse_heur Heures</span></td>";
        } elseif ($duree_sfaldina_rameuse_heur < "2") {
            echo "<td class='border-bottom-0'><span class='badge bg-success rounded-3 fw-semibold'>$duree_sfaldina_rameuse_heur Heures</span></td>";
        }
